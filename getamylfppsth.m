...
% LR='R';
% OFSort should set first
tic
%% should be setted ! added 180420
% OFS='OFSnev\';
OFS=[];
...
for i=3:numel(namelist)
    ...
    elseif namelist(i).isdir && strcmp(namelist(i).name(1:6),'ORICON')
        ...
        if strcmpi(LR,'reverse')
            CONblocknum=[CONblocknum; str2num(namelist(i).name(8:10))];
            CONLRlist=[CONLRlist namelist(i).name(7)];
        end
    end
end
if strcmpi(LR,'reverse')
    [~,blocksortidx]=sort(CONblocknum);
    ORICONlist=ORICONlist(blocksortidx);
    CONLRlist=CONLRlist(blocksortidx);
end
%%
strr='AmyConLFP'
if ~exist(['D:\data extracted\' destination(slashidx+1:end) '\amyconblockunitlfp.mat'],'file')
    amyconblockunitlfp=struct('timelabel',[],'lfp',[]);
    amyconblockunitlfp.lfp=nan(floor((conwin(2)-conwin(1))*fs*30/nsample),nblocktrial,ncnd,24,NORICON);
    if strcmpi(LR,'reverse')
        amyconblockunitlfp.LR=CONLRlist;
    else
    ...
    load(['D:\data extracted\' destination(slashidx+1:end) '\amyconblockunitlfp.mat']);
end
%%
strr='AmyOriLFP'
if ~exist(['D:\data extracted\' destination(slashidx+1:end)  '\amyoriblockunitlfp.mat'],'file')
    ...
    load(['D:\data extracted\' destination(slashidx+1:end)  '\amyoriblockunitlfp.mat']);
end
toc
%%
tic
...
if ~exist(['D:\data extracted\' destination(slashidx+1:end) '\amyconblockunitpsth.mat'],'file')
    amyconblockunitpsth=struct('psth',[],'LR',[],'spikevalid',zeros(24,NORICON),'stimvalid',zeros(24,NORICON));
    if strcmpi(LR,'reverse')
        amyconblockunitpsth.LR=CONLRlist;
    else
        amyconblockunitpsth.LR=LR;
    end
    amyconblockunitpsth.psth=nan(floor((conwin(2)-conwin(1))/0.001),nblocktrial,ncnd,24,NORICON);
    ...
    for n=1:numel(ORICONlist)
        for k=1:24
            path=[destination '\' OFS ORICONlist{n}];
            if exist([path '\Elec' num2str(elec(k)) 'Quality.mat'],'file')
                ...
                amyconblockunitpsth.spikevalid(k,n)=false;
                amyconblockunitlfp.spikevalid(k,n)=false;
            end
...
if ~exist(['D:\data extracted\' destination(slashidx+1:end) '\amyoriblockunitpsth.mat'],'file')
    ...
    for n=1:NDftORI
        for k=1:24
            path=[destination '\' OFS DftORIlist{n} ];
            if exist([path '\Elec' num2str(elec(k)) 'Quality.mat'],'file')
                load([path '\Elec' num2str(elec(k)) 'Quality.mat'])
                [~,snridx]=max(Quality.SNR);
...
save(['D:\data extracted\' destination(slashidx+1:end) '\amyconblockunitlfp.mat'],'amyconblockunitlfp','-v7.3');
save(['D:\data extracted\' destination(slashidx+1:end)  '\amyoriblockunitlfp.mat'],'amyoriblockunitlfp','-v7.3');
toc