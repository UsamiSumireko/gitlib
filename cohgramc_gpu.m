function [C,phi,S12,S1,S2,t,f,confC,phistd,Cerr]=cohgramc_gpu(data1,data2,movingwin,params)
...
nw=length(winstart);
if trialave;
   C=gpuArray(zeros(nw,Nf));
   S12=gpuArray(zeros(nw,Nf));
   S1=gpuArray(zeros(nw,Nf));
   S2=gpuArray(zeros(nw,Nf));
   phi=gpuArray(zeros(nw,Nf));
   Cerr=gpuArray(zeros(2,nw,Nf));
%    phierr=zeros(2,nw,Nf);
   phistd=zeros(nw,Nf);
else
   C=gpuArray(zeros(nw,Nf,Ch));
   S12=gpuArray(zeros(nw,Nf,Ch));
   S1=gpuArray(zeros(nw,Nf,Ch));
   S2=gpuArray(zeros(nw,Nf,Ch));
   phi=gpuArray(zeros(nw,Nf,Ch));
   Cerr=gpuArray(zeros(2,nw,Nf,Ch));
