...
allphip=[];
allphin=[];
str='D:\result\180129 filted rmmean coherence';
...
for iday=1:numel(folderlist)
    ...
    if strcmpi(CONORI,'con') 
        ...
        if strcmpi(spklfp{2},'lfp')
            ...
            V1blockunitdata.data=V1conblockunitlfp.lfp(:,:,:,V1selectidx,:);
%             V1blockunitdata.spikevalid=V1conblockunitlfp.spikevalid;
        elseif strcmpi(spklfp{2},'spk')
            ...
            V1blockunitdata.data=V1conblockunitpsth.psth(:,:,:,V1selectidx,:);
...
%         amystimvalid=amyblockunitlfp.stimvalid(:,1:2:n2b*2-1) & amyblockunitlfp.stimvalid(:,2:2:n2b*2);
    ...
    elseif strcmpi(CONORI,'ori')
        ...
        if strcmpi(spklfp{1},'lfp')
            load([folderlist{iday} '\amyoriblockunitlfp_30.mat']);
        ...
        if strcmpi(spklfp{2},'lfp')
            load([folderlist{iday} '\V1oriblockunitlfp_30.mat']);
            if V1selectlabel
                V1selectidx=logical(V1sigcell_ttest{iday});
            else
                V1selectidx=true(size(V1oriblockunitlfp.lfp,4),1);
            end
            V1blockunitdata.data=V1oriblockunitlfp.lfp(:,:,:,V1selectidx,:);
%             V1blockunitdata.spikevalid=V1oriblockunitlfp.spikevalid;
        elseif strcmpi(spklfp{2},'spk')
            ...
            V1blockunitdata.data=V1oriblockunitpsth.psth(:,:,:,V1selectidx,:);
...
%         amystimvalid=amyblockunitlfp.stimvalid;
    ...
    for nblock=1:size(amycellidx,2)
        ...
        for acell=1:numel(amyblockcellidx)
            acount=acount+1;
            if strcmpi(CONORI,'con')
                data1=reshape(amyrawdata(:,:,1:3,amyblockcellidx(acell),nblock),size(amyrawdata,1),[]);
                mdata1=repmat(mean(data1,2),1,size(data1,2));
                data1=data1-mdata1;
                data2=reshape(amyrawdata(:,:,multineucnd(:),amyblockcellidx(acell),nblock),size(amyrawdata,1),[]);
                mdata2=repmat(mean(data2,2),1,size(data2,2));
                data2=data2-mdata2;
            elseif strcmpi(CONORI,'ori')
                data1=reshape(amyrawdata(:,:,cndlist(1:3),amyblockcellidx(acell),nblock),size(amyrawdata,1),[]);
                mdata1=repmat(mean(data1,2),1,size(data1,2));
                data1=data1-mdata1;
                data2=reshape(amyrawdata(:,:,cndlist(4:6),amyblockcellidx(acell),nblock),size(amyrawdata,1),[]);
                mdata2=repmat(mean(data2,2),1,size(data2,2));
                data2=data2-mdata2;
            end
            if strcmpi(spklfp{1},'lfp')
        ...
        for vcell=1:size(v1rawdata,4)
            vcount=vcount+1;
            if strcmpi(CONORI,'con')
%                 data1=reshape(v1rawdata(:,:,1:3,vcell,nblock),size(v1rawdata,1),[]);
%                 mdata1=repmat(mean(data1,2),1,size(data1,2));
%                 data1=data1-mdata1;                
%                 data2=reshape(v1rawdata(:,:,multineucnd(:),vcell,nblock),size(v1rawdata,1),[]);
%                 mdata2=repmat(mean(data2,2),1,size(data2,2));
%                 data2=data2-mdata2;
                data1=squeeze(v1rawdata(:,:,1:3,vcell,nblock));
                mdata1=repmat(mean(data1,2),1,size(data1,2),1);
                data1=data1-mdata1;
                data1=reshape(data1,size(data1,1),[]);                
                data2=reshape(v1rawdata(:,:,multineucnd(:),vcell,nblock),size(v1rawdata,1),[],3);
                mdata2=repmat(mean(data2,2),1,size(data2,2),1);
                data2=data2-mdata2;
                data2=reshape(data2,size(data2,1),[]);
            elseif strcmpi(CONORI,'ori')
%                 data1=reshape(v1rawdata(:,:,cndlist(1:3),vcell,nblock),size(v1rawdata,1),[]);
%                 mdata1=repmat(mean(data1,2),1,size(data1,2));
%                 data1=data1-mdata1;
%                 data2=reshape(v1rawdata(:,:,cndlist(4:6),vcell,nblock),size(v1rawdata,1),[]);
%                 mdata2=repmat(mean(data2,2),1,size(data2,2));
%                 data2=data2-mdata2;
                data1=squeeze(v1rawdata(:,:,cndlist(1:3),vcell,nblock));
                mdata1=repmat(mean(data1,2),1,size(data1,2),1);
                data1=data1-mdata1;
                data1=reshape(data1,size(data1,1),[]);                    
                data2=squeeze(v1rawdata(:,:,cndlist(4:6),vcell,nblock));
                mdata2=repmat(mean(data2,2),1,size(data2,2),1);
                data2=data2-mdata2;
                data2=reshape(data2,size(data2,1),[]);    
            end
...
if sldwinlabel
    if trialshufflelabel
        figure
        plot_matrix(mean(mean(allpuff,4),3),t_t,t_f,'l');
        figure
        plot_matrix(mean(mean(allneu,4),3),t_t,t_f,'l');
        figure
        plot_matrix(mean(mean(allpuff,4),3)./mean(mean(allneu,4),3),t_t,t_f,'l');
    else
        figure
        plot_matrix(mean(allpuff,3),t_t,t_f,'l');
        figure
        plot_matrix(mean(allneu,3),t_t,t_f,'l');
        figure
        plot_matrix(mean(allpuff,3)./mean(allneu,3),t_t,t_f,'l');
    end
else
    if trialshufflelabel
        figure
